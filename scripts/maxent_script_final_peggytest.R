...
## can't do this because no connection, need real raster file
...
for (i in batch[1]:batch[length(batch)]) {
  ...
  ## FILE SAVING --------------------------------------
    dir.create("results")
    dir.create(paste0("results/", specs[i], "_model")) #save model outputs all in one place
    
    file.rename(from = "maxentResults.csv", to = paste0("results/", specs[i], "_maxentResults.csv"))
    write.csv(allbetas, paste0("results/", specs[i], "_allbetas.csv"))
    
    
    file.rename(from ="maxent.html", to = paste0("results/",specs[i], "_model/", specs[i], "_maxent.html"))
    file.rename(from = "plots/", to = paste0("results/",specs[i], "_model/", "plots/"))
    save(xm, file = paste(paste0("results/", specs[i], "_model/", specs[i]), bestbetas[i,"bestbeta"], "xm.RData", sep = '_'))
    ...
    write.csv(final.summary[i,], paste0("results/", specs[i], "_final_summary.csv"))
...
#final summary file
write.csv(final.summary, paste(batch[1], batch[length(batch)], "final.summary.csv", sep = "_")) #add specific batch name

system("rclone copy /scratch/projects/endophytedim/bestbetas.csv endodim:model_results/")
system("rclone copy /scratch/projects/endophytedim/final.summary.csv endodim:model_results/")
